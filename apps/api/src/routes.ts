...
router.get("/api/bot/dashboard",(_,res)=>{
  ...
  res.json({status:s.status,executionEnabled:process.env.EXECUTION_ENABLED==="true",
    walletBalanceSol:0,todayTrades:trades.length,todayProfitUsd:0,
    openPositions:active,consecutiveLosses:s.consecutiveLosses,lastEvent:s.lastEvent});
});
...
router.put("/api/bot/config",(req,res)=>{
  try {
    ...
    if(!validateTp(cfg)) return res.status(400).json({
